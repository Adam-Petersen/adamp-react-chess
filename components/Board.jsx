...
class Board extends React.Component {
  render() {
    ...
    for (let i = 0; i < 8; i++) {
      let col = [];
      for (let j = 0; j < 8; j++) {
        col.push(this.props.flip
          ? this.props.propsBoard[7 - i][7 - j] : this.props.propsBoard[i][j]);
      }
      board.push(col);
    }
                ...
                return (
                  <td key={`${i}${j}`}>
                    <Tile
                      ...
                      highlighted={this.props.highlightedTile
                        && this.props.highlightedTile.id === tile.id}
